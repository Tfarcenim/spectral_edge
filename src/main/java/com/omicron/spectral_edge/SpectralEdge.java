        ...
        if (target instanceof EntityLivingBase) {
            ...
            user.getHeldItemMainhand().getAttributeModifiers(EntityEquipmentSlot.MAINHAND).forEach((string, attribute) -> {
                if (attribute.getID().equals(ATTACK_DAMAGE_MODIFIER)) {
                    ...
                    float damageT = (float) ((damage / 100) * Config.spectralEdgePercentileDamageModifier * level);
                    if (user instanceof EntityPlayer) {
                        entitylivingbase.attackEntityFrom(DamageSource.causePlayerDamage((EntityPlayer) user),damageT);
                    } else {
                        entitylivingbase.attackEntityFrom(DamageSource.causeMobDamage(user), damageT);
                    }
                }
            });
        }
